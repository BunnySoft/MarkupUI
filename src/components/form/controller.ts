...
export function coordinateForm(form: HTMLFormElement, options: FormOptions): FormController {
  ...
  function elements(): HTMLFormControlsCollection { return Reflect.get(view!.HTMLFormElement.prototype, "elements", form) as HTMLFormControlsCollection }
  function nativeControls() { return [...elements()].filter(isControl) }
  function snapshot(): Snapshot {
    ...
    return { nodes: [form, ...elements()], fields, states: [
      String(Reflect.get(view!.HTMLFormElement.prototype, "noValidate", form)),
      ...[...elements()].map(node => {
  ...
  async function validate(settings: { keys?: readonly string[]; reason?: FormValidationReason } = {}): Promise<FormValidationResult> {
    ...
    try {
      ...
      if (settings.keys === undefined) issues.push(...nativeIssues([...elements()].filter(isValidatable)
  ...
  listen(document!, "invalid", event => {
    if (!isControl(event.target) || event.target.form !== form) return
    sync()
    if (!connected) return
    const item = items.find(item => item.controls.includes(event.target as FormControl))
