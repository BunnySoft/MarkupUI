...
export function selectOptions(control: HTMLSelectElement): HTMLOptionElement[] {
  const options = [...control.options], keys = new Set<string>()
  for (const option of options) {
    if (!option.hasAttribute("value") || keys.has(option.value)) throw new TypeError("Every option needs an explicit unique native string value.")
    if (option.parentElement !== control && !(option.parentElement?.localName === "optgroup" && option.parentElement.parentElement === control)) throw new TypeError("Use direct options or one native optgroup level.")
    keys.add(option.value)
  }
  for (const group of control.querySelectorAll("optgroup")) if (group.parentElement !== control || !group.label.trim()) throw new TypeError("Use direct optgroups with nonempty native labels.")
  return options
}
export function selectValue(control: HTMLSelectElement): SelectValue {
  return control.multiple ? [...control.options].filter(option => option.selected).map(option => option.value) : control.selectedIndex < 0 ? null : control.value
}
export function setSelectValue(control: HTMLSelectElement, value: string | null | readonly string[], apply = true, options = selectOptions(control)): void {
  const keys = control.multiple ? value : value === null ? [] : [value]
  if (!Array.isArray(keys) || !control.multiple && Array.isArray(value) || keys.some(key => typeof key !== "string")
  ...
  if (!apply) return
  if (control.multiple) for (const option of options) option.selected = keys.includes(option.value)
...
export function createSelect(root: HTMLElement, initialComposing = false, mutableMode = false): SelectController {
  ...
  const own = (node: Element) => node.closest("m-select,[data-select]") === root
  ...
  let connected = true, composing = initialComposing, pattern = "", error: string | null = null, generation = 0
  ...
  function label(node: HTMLSelectElement | HTMLInputElement) {
    return [...node.labels ?? []].some(item => item.textContent?.trim()) || node.getAttribute("aria-label")?.trim()
      || node.getAttribute("aria-labelledby")?.trim().split(/\s+/).every(id => document!.getElementById(id)?.textContent?.trim())
  }
  function validate() {
    ...
    if (control.multiple !== multiple) {
      if (!mutableMode) throw new TypeError("Select multiple mode changed; disconnect and create a new controller.")
    ...
    const next = selectOptions(control), nextGroups = [...control.querySelectorAll("optgroup")]
  ...
  const controller: SelectController = {
    control, filter, get connected() { return connected }, get error() { return error }, get composing() { return composing },
  ...
  listen(document!, "reset", event => {
    const resetsControl = event.target === control.form, resetsFilter = filter && event.target === filter.form
    if (resetsControl || resetsFilter) {
      generation++
      tasks.forEach(id => view!.clearTimeout(id)); tasks.clear()
      later(() => {
        if (!event.defaultPrevented && resetsFilter) composing = false
        attemptRefresh()
      })
    }
  }, true)
