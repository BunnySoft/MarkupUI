    ...
    for (const current of menus) {
      ...
      for (const node of current.querySelectorAll<HTMLElement>("*")) {
        ...
        if (node.matches(interactive) && !elements.includes(node)
          ...
          || node.localName.includes("-") && !isIconElement(node) && !isTypographyInline(node) && !(canonical && ["m-dropdown-item", "m-dropdown-group", "m-dropdown-divider"].includes(node.localName))
