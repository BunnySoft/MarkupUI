...
export interface DropdownOptions extends Omit<PopoverOptions, "trigger" | "delay" | "duration"> {
  ...
  onSelect?: (detail: DropdownSelection) => void
...
export interface DropdownController extends PopoverController {
  ...
  value: string | null
  refresh(): void
  configure(options: Pick<DropdownOptions, "placement" | "submenuDelay" | "submenuDuration" | "typeaheadDuration">): void
}
interface Item {
  element: HTMLElement
  key: string
...
const allowedOptions = new Set(["placement", "gap", "margin", "flip", "disabled", "positioning",
  "submenuDelay", "submenuDuration", "typeaheadDuration", "value", "onValue", "onSelect", "onError"])
...
export function createDropdown(trigger: HTMLElement, menu: HTMLElement, options: DropdownOptions = {}): DropdownController {
  ...
  const canonical = menu.localName === "m-dropdown-menu"
  const menuSelector = canonical ? "m-dropdown-menu" : "[data-dropdown-menu]"
  const itemSelector = canonical ? "m-dropdown-item > :is(button,a)" : "[data-dropdown-item]"
  const groupSelector = canonical ? "m-dropdown-group" : "[data-dropdown-group]"
  const dividerSelector = canonical ? "m-dropdown-divider" : "[data-dropdown-divider]"
  const keyOf = (element: Element) => canonical ? element.parentElement?.getAttribute("key") : element.getAttribute("data-dropdown-key")
  for (const key of Object.keys(options)) if (!allowedOptions.has(key)) throw new TypeError(`Unsupported Dropdown option: ${key}.`)
  let delay = options.submenuDelay ?? 100
  let duration = options.submenuDuration ?? 150
  let typeaheadDuration = options.typeaheadDuration ?? 500
  ...
  function label(node: HTMLElement): string {
    ...
    return ((canonical ? node.matches(itemSelector) ? node.parentElement?.getAttribute("label") : node.getAttribute("label") : node.getAttribute("data-dropdown-label")) ?? (references ? references.join(" ") : null)
      ?? node.getAttribute("aria-label") ?? node.textContent ?? "").trim()
  }
  function named(node: HTMLElement) {
    if (!(canonical && node.getAttribute("label")?.trim() || node.getAttribute("aria-label")?.trim() || node.getAttribute("aria-labelledby")?.trim()) || !label(node)) {
      throw new TypeError("Dropdown menus/groups need an explicit nonempty accessible name.")
    }
  }
  function report(error: unknown) {
    if (options.onError) options.onError(error)
    else menu.dispatchEvent(new view!.CustomEvent("m:dropdown-error", { detail: { error } }))
  }
  ...
  function applyValue() {
    ...
    for (const item of items.values()) {
      selected.attr(item.element, canonical ? "data-state" : "data-dropdown-selected", item.key === value ? canonical ? "selected" : "" : null)
    }
  ...
  function owns(state: MenuState, target: EventTarget | null): target is HTMLElement {
    return target instanceof view!.HTMLElement && target.closest(menuSelector) === state.menu
  }
  ...
  function select(state: MenuState, event: MouseEvent) {
    const target = event.target instanceof view!.Element ? event.target.closest<HTMLElement>(itemSelector) : null
    ...
    queue(() => {
      ...
      const path = [item.key]
      for (let parent = state; parent.parent; parent = parent.parent) path.unshift(keyOf(parent.trigger)!)
      value = item.key
      applyValue()
      ...
      const detail = { key: item.key, item: item.element, path, event }
      if (options.onSelect) options.onSelect(detail)
      else menu.dispatchEvent(new view!.CustomEvent<DropdownSelection>("m:dropdown-select", { detail }))
    }, state)
  }
  ...
  function build(initial: boolean) {
    if (!trigger.isConnected || !menu.isConnected || menu.getRootNode() !== document
      || !menu.matches(menuSelector) || trigger.contains(menu)) throw new TypeError("Dropdown needs connected separate light-DOM trigger/menu anatomy.")
    const menus = [menu, ...menu.querySelectorAll<HTMLElement>(menuSelector)]
    ...
    for (const current of menus) {
      if (!(canonical ? current.localName === "m-dropdown-menu" : ["ul", "ol"].includes(current.localName) && current.classList.contains("m-dropdown"))
      ...
      named(current)
      const parent = current.parentElement?.closest<HTMLElement>(menuSelector)
      const candidates = current === menu ? [rootTrigger] : [...parent!.querySelectorAll<HTMLButtonElement>(canonical ? "m-dropdown-item > button[popovertarget]" : "button[data-dropdown-item][popovertarget]")]
        .filter(button => button.closest(menuSelector) === parent && button.getAttribute("popovertarget") === current.id)
      ...
      const elements = [...current.querySelectorAll<HTMLElement>(itemSelector)].filter(element => element.closest(menuSelector) === current)
      const labels: string[] = []
      for (const element of elements) {
        const button = element instanceof view!.HTMLButtonElement
        const disabledAnchor = canonical && element instanceof view!.HTMLAnchorElement && element.parentElement?.hasAttribute("disabled") && !element.hasAttribute("href")
        const anchor = element instanceof view!.HTMLAnchorElement && (element.hasAttribute("href") || disabledAnchor)
        ...
        if (element.getAttribute("aria-disabled") === "true" && !disabledAnchor && (!button || !(element as HTMLButtonElement).disabled)) {
      ...
      for (const node of current.querySelectorAll<HTMLElement>("*")) {
        if (node.closest(menuSelector) !== current) continue
        const role = node.getAttribute("role")
        const allowedRole = elements.includes(node) ? ["menuitem"] : node.matches(groupSelector)
        ...
        if (node.matches(interactive) && !elements.includes(node)
          ...
          || node.localName.includes("-") && !isIconElement(node) && !isTypographyInline(node) && !isSpaceElement(node) && !(canonical && ["m-dropdown-item", "m-dropdown-group", "m-dropdown-divider"].includes(node.localName))
      ...
      for (const list of [current, ...current.querySelectorAll<HTMLElement>(groupSelector)]) {
        if (list !== current && list.closest(menuSelector) !== current) continue
        if (canonical ? [...list.children].some(node => !["m-dropdown-item", "m-dropdown-group", "m-dropdown-divider", "template"].includes(node.localName) && !(list !== current && node.getAttribute("data-part") === "group-label"))
          : !["ul", "ol"].includes(list.localName) || [...list.children].some(node => !["li", "template"].includes(node.localName))) {
          throw new TypeError("Menu/group lists require authored li wrappers.")
        }
        if (list !== current) named(list)
      }
      for (const divider of current.querySelectorAll<HTMLElement>(dividerSelector)) {
        if (divider.closest(menuSelector) === current
          && (canonical ? divider.childElementCount > 0 || !!divider.textContent?.trim()
    ...
    try {
      for (const record of records) {
        const parent = states.find(state => state.menu === record.trigger.closest(menuSelector)) ?? null
        ...
        const state: MenuState = { menu: record.menu, trigger: record.trigger, parent, core, disconnectCore: core.disconnect,
          items: record.elements.map((element, index) => ({ element, key: keyOf(element)!, label: record.labels[index]!, child: null })),
        ...
        if (canonical) {
          const position = core.syncPosition
          core.syncPosition = () => {
            const wasOpen = core.show, result = position()
            if (wasOpen && !result && !disabled && trigger.isConnected) report(new Error("Dropdown positioning is unavailable; the menu was closed."))
            return result
          }
        }
      ...
      for (const state of states) {
        ...
        if (canonical && state.menu.hasAttribute("label")) writes.attr(state.menu, "aria-label", state.menu.getAttribute("label"))
        if (supported) {
          writes.attr(state.menu, "tabindex", "-1")
          writes.attr(state.trigger, "aria-haspopup", "menu")
        }
        for (const node of state.menu.querySelectorAll<HTMLElement>(canonical ? "m-dropdown-item,m-dropdown-group,m-dropdown-divider" : "li, [data-dropdown-group]")) {
          if (node.closest(menuSelector) !== state.menu) continue
          writes.attr(node, "role", supported ? node.matches(dividerSelector) ? "separator" : node.matches(groupSelector) ? "group" : "none" : null)
          if (canonical && node.matches(groupSelector) && node.hasAttribute("label")) writes.attr(node, "aria-label", node.getAttribute("label"))
        }
      ...
      observer.observe(menu, { subtree: true, childList: true, attributes: true,
        attributeFilter: ["hidden", "inert", "disabled", "aria-disabled", "data-dropdown-key", "data-dropdown-label",
          ...
          "aria-label", "aria-labelledby", "contenteditable", "data-dropdown-item", "data-dropdown-menu", "data-dropdown-group", "dir", "key", "label"] })
    } catch (error) { destroy(); throw error }
  }
  const controller: DropdownController = {
    supported,
    get inline() { return !supported },
    get focusedElement() { return lastFocused },
    ...
    configure(next) {
      ...
      for (const number of [updated.submenuDelay ?? 100, updated.submenuDuration ?? 150, updated.typeaheadDuration ?? 500]) {
        if (!Number.isFinite(number) || number < 0 || number > 60000) throw new RangeError("Dropdown timing must be finite between 0 and 60000.")
      }
      options = updated
      delay = updated.submenuDelay ?? 100
      duration = updated.submenuDuration ?? 150
      typeaheadDuration = updated.typeaheadDuration ?? 500
      controller.refresh()
    },
    disconnect: destroy,
  }
