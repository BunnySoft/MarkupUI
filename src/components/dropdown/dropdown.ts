...
export class Dropdown extends ViewElement {
  ...
  private synchronize(strict = false): void {
    ...
    try {
      ...
      if ([...trigger.querySelectorAll("*")].some(node => node.localName.includes("-") && !isIconElement(node) && !isTypographyInline(node) && !isSpaceElement(node) || node.shadowRoot || ["script", "style", "slot"].includes(node.localName)
