...
import { validateCarouselGap } from "./model.js"
...
export interface CarouselOptions extends CarouselSettings {
  onChange?: (change: CarouselCurrentChangedDetail) => void
}
export interface CarouselController {
  ...
  validate(settings: CarouselSettings): void
...
const rootSelector = 'm-carousel,[data-part="carousel"]'
...
export function createCarousel(element: HTMLElement, options: CarouselOptions = {}): CarouselController {
  ...
  if (!view || !(element instanceof view.HTMLElement) || !element.matches(rootSelector)
    ...
    throw new TypeError("Use an unowned connected Carousel in the document, not a nested Carousel.")
  }
  ...
  const canonical = element.localName === "m-carousel"
  const viewports = find(canonical ? "m-carousel-viewport" : '[data-part="viewport"]')
  const controls = find(canonical ? "m-carousel-controls" : '[data-part="controls"]')
  const readouts = find(canonical ? "m-carousel-readout" : '[data-part="readout"]')
  ...
  if (canonical && (viewport.localName !== "m-carousel-viewport" || readout.localName !== "m-carousel-readout"
      || control && control.localName !== "m-carousel-controls")
    || viewport.parentElement !== element || !["div", "section", "m-carousel-viewport"].includes(viewport.localName) || !named(viewport)
  ...
  let settings: Required<CarouselSettings> = {
    currentIndex: 0, defaultIndex: 0, direction: "horizontal", gap: 0,
  ...
  let target: number | null = null, reason: CarouselCurrentChangedDetail["reason"] = "scroll", silent = false
  ...
  function config(input: CarouselSettings) {
    ...
    if (next.autoplay && !buttons.some(button => button.dataset.part === "toggle" && button.isConnected && control?.contains(button))) throw new TypeError("Autoplay requires an authored labelled pause/play button.")
    return next
  }
  function anatomy() {
    ...
    if (items.some(node => !(node instanceof win.HTMLElement) || !["div", "section", "article", "m-carousel-item"].includes(node.localName)
      || canonical && node.localName !== "m-carousel-item"
      || node.localName !== "m-carousel-item" && node.getAttribute("data-part") !== "item" || !own(node) || node.hasAttribute("hidden") || node.hasAttribute("inert")
      || node.hasAttribute("aria-hidden") || node.hasAttribute("role") && node.getAttribute("role") !== "group")) {
      throw new TypeError("Viewport children must be visible CarouselItem nodes.")
    }
    const itemKeys = new Set<string>()
    for (const item of items) {
      const key = item.getAttribute("key")
      if (key === null) continue
      if (!key.trim() || itemKeys.has(key)) throw new TypeError("CarouselItem keys must be unique and nonempty.")
      itemKeys.add(key)
    }
    const actions = control ? [...control.querySelectorAll<HTMLElement>('[data-part="prev"],[data-part="next"],[data-part="to"],[data-part="toggle"]')].filter(own) : []
    const seen = new Set<string>()
    for (const node of actions) {
      const action = node.dataset.part
      const key = action === "to" ? `to:${node.dataset.index}` : action!
      if (!(node instanceof win.HTMLButtonElement) || !control?.contains(node) || node.getAttribute("type") !== "button"
        ...
        || !(named(node) || node.textContent?.trim()) || seen.has(key)
        || action === "to" && (!/^(0|[1-9]\d*)$/.test(node.dataset.index ?? "") || !Number.isSafeInteger(Number(node.dataset.index)))) {
        throw new TypeError("Use unique labelled type=button controls; indicators need nonnegative decimal indices.")
      }
      seen.add(key)
  ...
  function geometry() {
    ...
    if (points.some((point, i) => !Number.isFinite(point) || Math.abs(point - i * (extent + settings.gap) * sign) > Math.max(2, i)
  ...
  function pauseReasons() {
    ...
    const rotation = buttons.find(button => button.dataset.part === "toggle")
  ...
  function unavailable(button: HTMLButtonElement) {
    ...
    return settings.disabled || slides.length < 2 || !geometry()
      || button.dataset.part === "prev" && !settings.loop && base <= 0
      || button.dataset.part === "next" && !settings.loop && base >= slides.length - 1
      || button.dataset.part === "to" && Number(button.dataset.index) >= slides.length
  }
  function render() {
    writes.attr(element, "role", "region"); writes.attr(element, "aria-roledescription", "carousel")
    writes.attr(element, "data-state", settings.direction)
    writes.attr(viewport, "data-part", "viewport")
    writes.style(viewport, "gap", `${settings.gap}px`)
    writes.attr(readout, "data-part", "readout")
    ...
    slides.forEach((slide, i) => {
      ...
      itemWrites.attr(slide, "data-part", "item")
      itemWrites.attr(slide, "data-index", String(i))
      itemWrites.attr(slide, "data-state", [i === index ? "current" : "", i === previousIndex ? "previous" : "", i === nextIndex ? "next" : ""].filter(Boolean).join(" "))
      if (!named(slide)) itemWrites.attr(slide, "aria-label", `${i + 1} of ${slides.length}`)
    })
    if (control) {
      writes.attr(control, "data-part", "controls")
      writes.attr(control, "hidden", null)
    }
    for (const button of buttons) {
      const toggle = button.dataset.part === "toggle"
      ...
      if (button.dataset.part === "to") writes.attr(button, "aria-current", Number(button.dataset.index) === index ? "true" : null)
      if (toggle) writes.attr(button, "aria-label", userPaused ? "Play slide rotation" : "Pause slide rotation")
    }
    const snapshot = state()
    writes.attr(element, "data-state", [settings.direction, snapshot.ready ? "ready" : "", snapshot.playing ? "playing" : "",
      userPaused ? "paused" : "", settings.disabled ? "disabled" : "", target !== null ? "pending" : ""].filter(Boolean).join(" "))
  }
  function schedule() {
  ...
  function change(next: number, why: CarouselCurrentChangedDetail["reason"], quiet: boolean) {
    ...
    const detail: CarouselCurrentChangedDetail = Object.freeze({ index, previousIndex, item: current, previousItem: previousSlide, reason: why })
  ...
  function command(value: number, why: CarouselCurrentChangedDetail["reason"], quiet = false, instant = false) {
    live()
  ...
  function refresh() {
    ...
    if (settings.autoplay && !next.actions.some(button => button.dataset.part === "toggle")) throw new TypeError("Retain the autoplay pause/play control.")
    ...
    function identity(item: HTMLElement | null | undefined) {
      if (item && slides.includes(item)) return slides.indexOf(item)
      const key = item?.getAttribute("key")
      return key ? slides.findIndex(slide => slide.getAttribute("key") === key) : -1
    }
    const retained = identity(prior), pending = identity(requested)
    index = retained >= 0 ? retained : clamp(index)
    const desired = pending >= 0 ? pending : index
  ...
  const { onChange: _callback, ...input } = options
  settings = config(input)
  if (typeof viewport.scrollTo !== "function") throw new Error("Native Element.scrollTo is required.")
  ...
  listen(viewport, "wheel", event => { if (event.target instanceof win.Element && event.target.closest(rootSelector) === element) interrupt() }, { passive: true })
  ...
  listen(element, "click", event => {
    ...
    if (node.dataset.part === "toggle") { userPaused = !userPaused; schedule() }
    else command(node.dataset.part === "to" ? Number(node.dataset.index) : (target ?? index) + (node.dataset.part === "next" ? 1 : -1), "control")
  })
  render(); observers()
  ...
  return {
    ...
    set(input) {
      ...
      const pending = target !== null, why = reason, quiet = silent
      settings = next; target = null; interacting = false
      render(); observers()
      if (!settings.disabled) command(clamp(desired), pending && input.currentIndex === undefined ? why : "api",
        pending && input.currentIndex === undefined ? quiet : true, true)
      else schedule()
    },
    reset() { command(clamp(settings.defaultIndex), "api", false, true) },
    refresh, disconnect,
  }
}
