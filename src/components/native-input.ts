/** Internal native-field enhancement shared by Input and unmigrated native-control compositions. */
...
export function createInput(root: HTMLElement, options: InputOptions = {}, requireName = true, initialComposing = false): InputController {
  ...
  function anatomy() {
    if (!valid() || requireName && !named(control) || control.hasAttribute("role")) throw new TypeError("Keep the authored native field named and in its original Input root.")
  ...
  anatomy()
  if (revealButton && (!(control instanceof view.HTMLInputElement) || requireName && control.type !== "password")) {
    throw new TypeError("Password reveal requires an authored input[type=password].")
  }
  let connected = true, composing = initialComposing, visible = false, generation = 0
  ...
  if (clearButton) listen(clearButton, "click", event => {
    ...
    later(() => { if (pending === generation && !event.defaultPrevented && !clearButton.disabled && !clearButton.hidden && !clearButton.matches(":disabled")) clear() })
  })
  ...
  const controller: InputController = { control, get connected() { return connected }, get composing() { return composing }, refresh, setValue, clear, disconnect }
