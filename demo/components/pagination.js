...
nav.addEventListener("m:pagination-error", event => { state.textContent = event.detail.error.message })
...
document.querySelector("#cancel").addEventListener("click", () => {
  nav.addEventListener("m:pagination-request", event => { event.preventDefault(); state.textContent = "Author cancelled the local paging request." }, { once: true })
})
