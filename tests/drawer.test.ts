...
function fixture() {
  ...
  host.innerHTML = `<button data-opener type="button">Open</button><dialog class="m-native-dialog m-drawer" aria-label="Drawer details"><form class="m-drawer-content" method="dialog"><header data-drawer-header><h3 data-drawer-title>Authored level three</h3></header><div data-drawer-body tabindex="0" aria-label="Details"><div data-drawer-body-content><label>Reference <input required name="reference"></label><p>Details</p></div></div><footer data-drawer-footer><button value="saved">Save</button><button value="closed" formnovalidate>Close</button><button value="cancelled" formnovalidate>Cancel</button></footer></form></dialog>`
...
describe("Drawer and CSS-only DrawerContent", () => {
  ...
  it("rejects invalid/missing native anatomy and duplicate companion regions", () => {
    for (const fault of ["class", "body", "duplicate", "placement"]) {
      const d = fixture()
      if (fault === "class") d.classList.remove("m-drawer")
...
describe("Drawer backdrop and template owners", () => {
  ...
  it("guards reentrant creation and native focus-triggered owner disposal", () => {
    ...
    c.dialog.addEventListener("m:native-dialog-dispose", () => { try { o.create(template()) } catch { blocked = true } })
  ...
  it("ships native edge/body CSS without custom resizer, scroll-lock or animation dependencies", () => {
    ...
    expect(css).toContain("height:min(var(--m-drawer-height")
    expect(css).toContain("dialog.m-native-dialog.m-drawer")
  ...
  it("uses measured default dimensions, region padding and header typography", () => {
    const css = drawerCSS()
    expect(css).toContain("width:min(var(--m-drawer-width,251px),100%)")
    expect(css).toContain("height:min(var(--m-drawer-height,251px),100%)")
    expect(css).toContain("padding:var(--m-drawer-padding,16px 24px)")
  ...
  it("keeps physical and logical corner mappings private and author overrides intact", () => {
    ...
    expect(css).toContain("border-radius:var(--m-drawer-radius,var(--_drawer-radius,3px 0 0 3px))")
    ...
    for (const name of ["width", "height", "padding", "border", "radius", "color", "background", "focus"]) {
      expect(css).not.toMatch(new RegExp(`--m-drawer-${name}:`))
    }
  })
  it("uses Drawer-specific neutral paint and mask without weakening repeated-base guards", () => {
    ...
    expect(css).toContain("dialog.m-native-dialog.m-drawer::backdrop{background:rgb(0 0 0 / 30%)}")
  ...
  it("preserves fixed modality, control and scrolling safety, and all-mode print resets", () => {
    const css = drawerCSS()
    expect(css).toContain("dialog.m-drawer:modal{position:fixed;top:0;bottom:0;right:0;left:auto;height:100%;margin:0}")
    ...
    expect(css).toContain("@media print{dialog.m-native-dialog.m-drawer[open]{position:static;inset:auto;width:100%;height:auto;max-block-size:none;margin:0;box-shadow:none}")
    expect(css).toContain("dialog.m-native-dialog.m-drawer::backdrop{background:transparent}")
  })
