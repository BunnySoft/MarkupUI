...
describe("Progress default styles", () => {
  it("uses the info fill palette and scoped neutral rails instead of legacy primary colors", () => {
    withRules((rules) => {
      const defaults = rules.find((rule) => (rule as CSSStyleRule).selectorText === "m-progress[data-m-progress]") as CSSStyleRule
      expect(defaults.style.getPropertyValue("--_m-progress-tone")).toBe("var(--m-color-info, #2080f0)")
      ...
      expect(light.style.getPropertyValue("--_m-progress-default-rail")).toBe("#ebebeb")
      expect(dark.style.getPropertyValue("--_m-progress-default-rail")).toBe("rgba(255, 255, 255, .12)")
  ...
  it("replaces decorative status glyphs with scalable masks without hiding inside or unknown values", () => {
    withRules((rules) => {
      const masks = rules.filter((rule) => (rule as CSSStyleRule).style?.getPropertyValue("--_m-progress-status-mask").startsWith("url(")) as CSSStyleRule[]
      expect(masks).toHaveLength(4)
      expect(masks.every((rule) => rule.style.getPropertyValue("--_m-progress-status-mask").includes("data:image/svg+xml"))).toBe(true)
      const replacement = rules.find((rule) => (rule as CSSStyleRule).style?.getPropertyValue("font-size") === "0") as CSSStyleRule
      expect(replacement.selectorText).toContain("[data-m-progress-indeterminate]")
      expect(replacement.selectorText).toContain('[data-m-progress-placement="inside"]')
      expect(replacement.selectorText).toContain('[data-m-progress-type="multiple-circle"]')
    })
  })

  it("matches the two-second processing sweep and disables motion under reduced motion", () => {
    withRules((rules) => {
      const sweep = rules.find((rule) => (rule as CSSKeyframesRule).name === "m-progress-shine") as CSSKeyframesRule
      ...
      expect(frames[1]?.style.getPropertyValue("width")).toContain("--_m-progress-percent")
      const processing = rules.find((rule) => (rule as CSSStyleRule).style?.getPropertyValue("animation").includes("m-progress-shine")) as CSSStyleRule
      expect(processing.style.getPropertyValue("animation")).toBe("m-progress-shine 2s cubic-bezier(.4, 0, .2, 1) infinite")
