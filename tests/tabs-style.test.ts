...
describe("Tabs retained default styles", () => {
  ...
  it("defines private palette fallbacks while retaining correct shared primary-color ownership", () => {
    const rules = install()
    const themeRules = rules.filter(rule => rule.selectorText?.includes("data-m-theme"))
    expect(themeRules).toHaveLength(2)
    for (const rule of themeRules) {
      for (let i = 0; i < rule.style.length; i++) expect(rule.style[i]).toMatch(/^--_tabs-/)
    }
    expect(css).toContain("var(--m-color-primary, var(--_tabs-primary, #18a058))")
  ...
  it("keeps size/type defaults private so public padding remains authoritative", () => {
    document.body.innerHTML = '<div class="m-tabs m-tabs--large" data-tabs-type="card" style="--m-tabs-tab-padding:7px 13px"><button data-tabs-tab type="button">Tab</button></div>'
    const rules = install()
    const root = document.querySelector(".m-tabs")!
    ...
    expect(getComputedStyle(root).getPropertyValue("--m-tabs-tab-padding")).toBe("7px 13px")
    expect(css).toContain("var(--m-tabs-tab-padding, var(--_tabs-pad))")
    expect(css).toContain("--_tabs-gap: 36px")
    expect(css).toContain("--_tabs-gap: 4px")
    expect(css).toContain("--_tabs-gap: 0")
    const small = rules.find(rule => rule.selectorText === ".m-tabs--small")!
    expect(small.style.getPropertyValue("--m-tabs-tab-padding")).toBe("")
  })

  it("uses native disabled buttons without fading their entire border/background", () => {
    document.body.innerHTML = '<div class="m-tabs"><button type="button" data-tabs-tab disabled>Unavailable</button></div>'
    const before = document.body.innerHTML
    const rules = install()
    const disabled = rules.find(rule => rule.selectorText === ".m-tabs [data-tabs-tab]:disabled")!
    expect(disabled.style.getPropertyValue("color")).toContain("--m-tabs-disabled")
  ...
  it("keeps panes unboxed by default and preserves authored first-child spacing", () => {
    document.body.innerHTML = '<div class="m-tabs"><section data-tabs-pane><h2 style="margin-block-start:17px">Authored heading</h2></section></div>'
    const rules = install()
    const pane = rules.find(rule => rule.selectorText === ".m-tabs [data-tabs-pane]")!
  ...
  it("uses local 2px indicators for line/bar and no extra underline for card/segment", () => {
    const rules = install()
    const indicator = rules.find(rule => rule.selectorText?.replaceAll('"', "") === '.m-tabs [data-tabs-tab][aria-selected=true]::after')!
  ...
  it("maps pane padding to physical placements and logical start/end under RTL", () => {
    ...
    const pane = rules.find(rule => rule.selectorText === ".m-tabs [data-tabs-pane]")!
    expect(pane.style.getPropertyValue("padding")).toBe("var(--m-tabs-pane-padding, var(--_tabs-p))")
    const mappings = [
      ['.m-tabs[data-tabs-placement=bottom][data-tabs-enhanced]', "0 0 var(--_tabs-i)"],
      ['.m-tabs:is([data-tabs-placement=left],[data-tabs-placement=start])[data-tabs-enhanced]', "0 0 0 var(--_tabs-i)"],
      ['.m-tabs:is([data-tabs-placement=right],[data-tabs-placement=end])[data-tabs-enhanced]', "0 var(--_tabs-i) 0 0"],
      ['.m-tabs:is([data-tabs-placement=left],[data-tabs-placement=end]):dir(rtl)[data-tabs-enhanced]', "0 0 0 var(--_tabs-i)"],
      ['.m-tabs:is([data-tabs-placement=right],[data-tabs-placement=start]):dir(rtl)[data-tabs-enhanced]', "0 var(--_tabs-i) 0 0"]
    ]
    ...
    expect(rules.find(rule => rule.selectorText === ".m-tabs")!.style.getPropertyValue("--_tabs-i")).toBe("calc(12px + var(--_tabs-size) * 4px)")
  })

  it("uses the distinct vertical-card density without overriding public padding", () => {
    const rules = install()
    expect(rules.find(rule => rule.selectorText === ".m-tabs")!.style.getPropertyValue("--_tabs-v")).toBe("8px")
    expect(rules.find(rule => rule.selectorText === ".m-tabs[data-tabs-type=card]")!.style.getPropertyValue("--_tabs-v")).toBe("10px")
    const vertical = rules.find(rule => rule.selectorText === ".m-tabs [aria-orientation=vertical] [data-tabs-tab]")!
    expect(vertical.style.getPropertyValue("padding")).toBe("var(--m-tabs-tab-padding, calc(var(--_tabs-v) + var(--_tabs-size) * 2px) calc(16px + var(--_tabs-size) * 4px))")
  })

  it("never applies selected foreground to a disabled tab, including forced colors", () => {
    document.body.innerHTML = '<div class="m-tabs"><button type="button" data-tabs-tab aria-selected="true" disabled>Transitioning</button></div>'
    const rules = install()
