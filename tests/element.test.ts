...
describe("Element resolved through authored native HTML", () => {
  it("adds no wrapper, factory, export, dependency or distribution", () => {
    ...
    expect(html).not.toMatch(/<m-|role="(?:none|presentation|button|link|heading)"|\sstyle=|<style>/)
  })
  it("reuses the accepted application palettes instead of defining a second palette", () => {
    ...
    expect(css).toContain("var(--m-bg-surface, Canvas)")
    expect(css).toContain("var(--m-color-primary, LinkText)")
    expect(css).not.toMatch(/--m-[\w-]+\s*:/)
  ...
  it("disconnects only its listeners, keeping author listeners, inline overrides and values", () => {
    ...
    connect(root)
    button.click()
    root.querySelector<HTMLElement>("#element-scope")!.style.setProperty("--m-color-primary", "#123456")
    connections[0].disconnect()
    button.click()
    expect(author).toHaveBeenCalledTimes(2)
    expect(root.querySelector("[data-count]")!.textContent).toBe("1")
    expect(root.querySelector<HTMLElement>("#element-scope")!.style.getPropertyValue("--m-color-primary")).toBe("#123456")
    connections[0].disconnect()
