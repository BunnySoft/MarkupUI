...
describe("component-by-component demo browser", () => {
  ...
  it("lists exactly the 98 existing standalone pages without group pages", () => {
    const files = readdirSync(resolve("demo", "components")).filter(file => file.endsWith(".html")).sort()
    expect(components).toHaveLength(98)
    expect(new Set(components.map(item => item.slug)).size).toBe(98)
    ...
    expect(document.querySelectorAll("a[data-component]")).toHaveLength(98)
  ...
  it("filters actual component items and hides empty categories", () => {
    ...
    expect(document.querySelector("#component-count")?.textContent).toBe("1 of 98 components")
