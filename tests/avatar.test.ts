...
import { MAvatar, MAvatarGroup, registerAvatar } from "../src/components/avatar/index.js"
...
function avatar(markup: string): MAvatar {
  document.body.innerHTML = markup
  const element = document.querySelector("m-avatar")
  if (!(element instanceof MAvatar)) throw new Error("Avatar was not upgraded")
  return element
}
...
describe("standalone Avatar", () => {
  it("adopts authored image and fallback nodes without discarding listeners", () => {
    ...
    expect(element.querySelector<HTMLElement>("[data-m-avatar-content]")?.hidden).toBe(true)
  })

  it("handles source changes, native lazy loading and bounded fallback attempts", () => {
    const element = avatar('<m-avatar src="/first.png" fallback-src="/fallback.png" lazy alt="Ada">A</m-avatar>')
    ...
    expect(element.dataset.mAvatarState).toBe("loading")
    image.dispatchEvent(new Event("error"))
    expect(element.dataset.mAvatarState).toBe("error")
    ...
    expect(element.dataset.mAvatarState).toBe("empty")
  })

  it("clones inert placeholder/fallback templates and changes visibility with state", () => {
    const element = avatar(`<m-avatar src="/missing.png" alt="Ada">
      A
      <template data-m-avatar-placeholder><span>Loading</span></template>
      <template data-m-avatar-fallback><strong>Unavailable</strong></template>
    </m-avatar>`)
    const placeholder = element.querySelector<HTMLElement>(":scope > span[data-m-avatar-placeholder]")!
    const fallback = element.querySelector<HTMLElement>(":scope > span[data-m-avatar-fallback]")!
  ...
  it("adopts late content and preserves explicit accessibility labels", async () => {
    ...
    expect(element.querySelector("[data-m-avatar-content]")?.contains(content)).toBe(true)
  ...
  it("restores authored loading behavior after reconnection", () => {
    const element = avatar('<m-avatar lazy><img src="/ada.png" alt="Ada" loading="eager"></m-avatar>')
    const image = element.querySelector("img")!
    expect(image.getAttribute("loading")).toBe("lazy")
    element.remove()
    document.body.append(element)
  ...
  it("reconciles an image that failed while disconnected", () => {
    const element = avatar('<m-avatar src="/bad.png"><template data-m-avatar-fallback><span>Fallback</span></template></m-avatar>')
    const image = element.querySelector("img")!
    element.remove()
    ...
    document.body.append(element)
    expect(element.dataset.mAvatarState).toBe("error")
    expect(element.querySelector<HTMLElement>(":scope > span[data-m-avatar-fallback]")?.hidden).toBe(false)
  })
  ...
  it("honors properties assigned before element upgrade", () => {
    ...
    document.body.append(element)
    customElements.define("m-avatar-preupgrade", class extends MAvatar {})
  ...
  it("uses bounded numeric CSS values and preserves source-free text", () => {
    const element = avatar('<m-avatar alt="Ada" size="48" object-fit="contain">AB</m-avatar>')
...
describe("Avatar text fitting", () => {
  ...
  it("refits character data and nested child replacement while keeping the full accessible label", async () => {
    ...
    const text = element.querySelector<HTMLElement>("[data-m-avatar-content]")!
  ...
  it("refits numeric sizes and resize/font changes using untransformed width and height", () => {
    ...
    const text = element.querySelector("[data-m-avatar-content]")!
    element.size = 22
  ...
  it("fits only visible content, then recalculates when placeholder, fallback or text is revealed", () => {
    ...
    const element = avatar(`<m-avatar src="/missing.png">Alexandria
      <template data-m-avatar-placeholder>Loading profile</template>
      <template data-m-avatar-fallback>Unavailable profile</template>
    </m-avatar>`)
    const content = element.querySelector<HTMLElement>("[data-m-avatar-content]")!
    const placeholder = element.querySelector<HTMLElement>("span[data-m-avatar-placeholder]")!
    const fallback = element.querySelector<HTMLElement>("span[data-m-avatar-fallback]")!
  ...
  it("unobserves replaced content and disconnects all measurements until reconnection", async () => {
    ...
    const replacement = element.querySelector<HTMLElement>("[data-m-avatar-content]")!
  ...
  it("does not erase an authored size/fit override on unrelated synchronization", () => {
    measure()
    const element = avatar('<m-avatar size="52" object-fit="cover">Alexandria</m-avatar>')
...
describe("Avatar group", () => {
  it("uses a native details overflow while preserving avatar identity", () => {
    ...
    if (!(group instanceof MAvatarGroup)) throw new Error("Group was not upgraded")
    ...
    expect(group.querySelectorAll("[data-m-avatar-rest] > m-avatar")).toHaveLength(2)
  ...
  it("preserves order when adding or removing overflow items", async () => {
    ...
    group.querySelector("[data-m-avatar-rest] > m-avatar")!.remove()
    await Promise.resolve()
  ...
  it("replays max assigned before group upgrade", () => {
    ...
    customElements.define("m-avatar-group-preupgrade", class extends MAvatarGroup {})
    expect(group.querySelector("summary")?.textContent).toBe("+1")
