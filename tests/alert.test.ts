...
describe("standalone Alert", () => {
  it("preserves free-form native content and listeners without adding an interactive or live role", () => {
    const element = document.createElement("m-alert") as MAlert
    ...
    expect([...element.querySelector("[data-m-alert-content]")!.childNodes]).toEqual([text, button])
    button.click()
  ...
  it("preserves explicitly authored native roles, live settings and names", () => {
    const element = alert('<m-alert role="status" aria-live="polite" aria-atomic="true" aria-labelledby="heading" closable><h2 data-m-alert-header id="heading">Saved</h2><p data-m-alert-content>Initial</p></m-alert>')
    element.type = "error"
  ...
  it("does not add assertive semantics when a static notice is inserted or updated", async () => {
    const element = alert("<m-alert type='warning'>Initial</m-alert>")
    element.querySelector("[data-m-alert-content]")!.textContent = "Changed"
  ...
  it("adopts authored header/content/icon/action nodes without rewriting headings or controls", () => {
    const element = alert('<m-alert><span data-m-alert-icon aria-hidden="true">★</span><header data-m-alert-header><h3>Title</h3></header><section data-m-alert-content><div data-m-alert-actions><button type="button">Retry</button></div></section></m-alert>')
    const header = element.querySelector("header")!
    const content = element.querySelector("section")!
    expect(header.parentElement).toBe(element.querySelector("[data-m-alert-body]"))
    expect(content.parentElement).toBe(header.parentElement)
    expect(element.querySelector("[data-m-alert-icon]")?.parentElement).toBe(element)
  ...
  it("clears only the generated title when the title attribute is removed", () => {
    const element = alert('<m-alert title="Title"><strong>Body</strong></m-alert>')
    const body = element.querySelector("strong")
    element.removeAttribute("title")
    expect(element.querySelector("[data-m-alert-header]")).toBeNull()
  ...
  it("uses decorative native SVG semantic icons, with no empty default icon", () => {
    ...
    for (const type of ["info", "success", "warning", "error"]) {
      element.type = type
      const icon = element.querySelector("[data-m-alert-icon]")!
  ...
  it("does not rebuild generated SVGs on content or close-label changes", async () => {
    const element = alert('<m-alert type="info" closable>Before</m-alert>')
    const icon = element.querySelector("[data-m-alert-icon] svg")
    const glyph = close(element).querySelector("svg")
    element.querySelector("[data-m-alert-content]")!.textContent = "After"
    element.closeLabel = "Dismiss"
    await Promise.resolve()
    expect(element.querySelector("[data-m-alert-icon] svg")).toBe(icon)
    expect(close(element).querySelector("svg")).toBe(glyph)
    element.showIcon = false
    expect(element.querySelector("[data-m-alert-icon]")).toBeNull()
    element.showIcon = true
    expect(element.querySelectorAll("[data-m-alert-icon] svg")).toHaveLength(1)
  })

  it("preserves authored icon identity, listeners and ARIA across type/show changes", () => {
    const element = document.createElement("m-alert") as MAlert
    const icon = document.createElement("span")
    icon.setAttribute("data-m-alert-icon", "")
    ...
    element.type = "error"
    element.showIcon = false
    expect(element.querySelector("[data-m-alert-icon]")).toBe(icon)
    expect(icon.getAttribute("aria-label")).toBe("Custom symbol")
    expect(icon.hasAttribute("aria-hidden")).toBe(false)
    element.showIcon = true
    icon.click()
    expect(click).toHaveBeenCalledOnce()
    expect(element.querySelectorAll("[data-m-alert-icon]")).toHaveLength(1)
  })

  it("adopts a late custom icon without retaining the generated one", async () => {
    const element = alert('<m-alert type="info">Body</m-alert>')
    const generated = element.querySelector("[data-m-alert-icon]")
    const icon = document.createElement("span")
    icon.dataset.mAlertIcon = ""
    icon.textContent = "★"
    element.append(icon)
    await Promise.resolve()
    expect(element.querySelectorAll("[data-m-alert-icon]")).toHaveLength(1)
  ...
  it("accepts direct native SVG icons without moving them into body content", () => {
    const element = alert('<m-alert type="info"><svg data-m-alert-icon aria-hidden="true" viewBox="0 0 20 20"><circle cx="10" cy="10" r="8"></circle></svg>Body</m-alert>')
    ...
    expect(element.querySelector("[data-m-alert-content]")?.textContent).toBe("Body")
  })

  it("preserves inert templates even when they carry region markers", () => {
    const element = alert('<m-alert><template data-m-alert-header><h2>Inert header</h2></template><template data-m-alert-icon><span>Inert icon</span></template>Body</m-alert>')
    expect(element.querySelectorAll(":scope > template")).toHaveLength(2)
    expect(element.querySelector("h2")).toBeNull()
    expect(element.querySelector("[data-m-alert-body]")?.textContent).toBe("Body")
    expect(element.querySelector(":scope > span[data-m-alert-icon]")).toBeNull()
  })

  it("adopts late explicit content and preserves original body nodes", async () => {
    const element = alert("<m-alert><strong>Original</strong></m-alert>")
    const original = element.querySelector("strong")
    const content = document.createElement("section")
    content.dataset.mAlertContent = ""
    content.textContent = "New"
    element.append(content)
    await Promise.resolve()
    expect(element.querySelectorAll("[data-m-alert-content]")).toHaveLength(1)
  ...
  it("adopts an authored body arriving after generated structure", async () => {
    const element = alert('<m-alert title="Title" closable>Original</m-alert>')
    const nativeClose = close(element)
    const body = document.createElement("div")
    body.dataset.mAlertBody = ""
    const header = document.createElement("h2")
    header.dataset.mAlertHeader = ""
    header.textContent = "Authored"
    body.append(header)
    element.append(body)
    await Promise.resolve()
    expect(element.querySelectorAll("[data-m-alert-body]")).toHaveLength(1)
    expect(element.querySelector("[data-m-alert-body]")).toBe(body)
    expect(element.querySelector("[data-m-alert-header]")).toBe(header)
  ...
  it("emits cancellable close intent without hiding, removal or leave notifications", () => {
    const element = alert('<m-alert closable title="Notice">Body</m-alert>')
    const intent = vi.fn((event: Event) => event.preventDefault())
    const leave = vi.fn()
    element.addEventListener("m:close", intent)
    element.addEventListener("m:after-leave", leave)
    element.addEventListener("m:after-hide", leave)
    close(element).click()
  ...
  it("does not remove the alert when close intent is not cancelled", () => {
    const element = alert("<m-alert closable>Body</m-alert>")
    close(element).click()
    expect(element.isConnected && !element.hidden).toBe(true)
  })

  it("uses an accessible native type=button close without submitting the enclosing form", () => {
    const element = alert('<form><m-alert closable close-label="Dismiss warning"><button type="submit">Retry</button></m-alert></form>')
    ...
    element.querySelector<HTMLButtonElement>("[data-m-alert-content] button")!.click()
  ...
  it("updates close labels without touching original content or control identity", () => {
    const element = alert("<m-alert closable>Body</m-alert>")
    const native = close(element)
    const content = element.querySelector("[data-m-alert-content]")
    ...
    expect(element.querySelector("[data-m-alert-content]")).toBe(content)
  })

  it("does not rewrite ARIA or move controls during authored live-region text updates", async () => {
    const element = alert('<m-alert role="status" closable><p data-m-alert-content>Before</p></m-alert>')
  ...
  it("removes stale close listeners when closable is unset", () => {
    ...
    native.click()
    expect(intent).not.toHaveBeenCalled()
    expect(element.querySelector("[data-m-alert-close]")).toBeNull()
  ...
  it("preserves native disabled fieldsets and explicitly disabled close controls", () => {
    const element = alert("<fieldset disabled><m-alert closable>Body</m-alert></fieldset>")
    const intent = vi.fn()
    element.addEventListener("m:close", intent)
    const native = close(element)
  ...
  it("releases observers/listeners on disconnect and reconnects without losing native state", async () => {
    const element = alert('<m-alert closable title="Before"><input value="Initial"></m-alert>')
    const native = close(element)
    ...
    await Promise.resolve()
    expect(intent).not.toHaveBeenCalled()
    expect(element.querySelector("[data-m-alert-header]")?.textContent).toBe("Before")
    document.body.append(element)
    expect(close(element)).toBe(native)
    expect(element.querySelector("input")).toBe(input)
    expect(input.value).toBe("Edited")
    expect(element.querySelector("[data-m-alert-header]")?.textContent).toBe("After")
    native.click()
    expect(intent).toHaveBeenCalledOnce()
  })

  it("handles replaced regions without resurrecting discarded authored content", async () => {
    const element = alert('<m-alert closable title="Fallback"><header data-m-alert-header>Old header</header><p data-m-alert-content>Old body</p></m-alert>')
    const old = element.querySelector("p")!
    const native = close(element)
    element.innerHTML = "<h3 data-m-alert-header>New header</h3><p data-m-alert-content>New body</p>"
    await Promise.resolve()
    expect(close(element)).toBe(native)
    expect(element.contains(old)).toBe(false)
    expect(element.querySelectorAll("[data-m-alert-body]")).toHaveLength(1)
    expect(element.querySelectorAll("[data-m-alert-header]")).toHaveLength(1)
    expect(element.textContent).not.toContain("Old")
  })

  it("does not emit a second parent close for nested alerts", () => {
    const outer = alert('<m-alert closable><m-alert closable>Inner</m-alert></m-alert>')
    const inner = outer.querySelector("m-alert") as MAlert
    const intent = vi.fn()
  ...
  it("preserves pre-definition properties and runtime CSS separation", () => {
    ...
    const element = document.querySelector("test-late-alert") as MAlert
    ...
    customElements.define("test-late-alert", class extends MAlert {})
    expect(element.querySelector("p")).toBe(content)
    expect(element.querySelector("[data-m-alert-header]")?.textContent).toBe("Late")
