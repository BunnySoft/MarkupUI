...
describe("Skeleton default styles", () => {
  it("animates the two color endpoints at the upstream phases without fading opacity", () => {
    withRules((rules) => {
      ...
      expect(pulse.name).toBe("m-skeleton-pulse")
      ...
      expect(frames[0]?.style.getPropertyValue("background-color")).toContain("--m-skeleton-color,")
      expect(frames[1]?.style.getPropertyValue("background-color")).toContain("--m-skeleton-color-end,")
  ...
  it("shares the default animation between static spans and generated bars", () => {
    withRules((rules) => {
      const surface = rules.find((rule) => (rule as CSSStyleRule).style?.getPropertyValue("animation").includes("m-skeleton-pulse")) as CSSStyleRule
      expect(surface.selectorText).toContain(".m-skeleton")
      expect(surface.selectorText).toContain("[data-m-skeleton-item]")
      expect(surface.style.getPropertyValue("animation")).toBe("m-skeleton-pulse 2s cubic-bezier(.36, 0, .64, 1) infinite")
      expect(surface.style.getPropertyValue("background")).toContain("--m-skeleton-color")
    })
  })
  ...
  it("defines scoped light and dark endpoints and stops motion in reduced-motion mode", () => {
    withRules((rules) => {
      ...
      expect(light.style.getPropertyValue("--_m-skeleton-color-start")).toBe("#eee")
      expect(light.style.getPropertyValue("--_m-skeleton-color-end")).toBe("#ddd")
      expect(dark.style.getPropertyValue("--_m-skeleton-color-start")).toBe("rgba(255, 255, 255, .12)")
      expect(dark.style.getPropertyValue("--_m-skeleton-color-end")).toBe("rgba(255, 255, 255, .18)")
