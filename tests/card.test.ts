...
function card(markup = "<m-card>Content</m-card>"): MCard {
  document.body.innerHTML = markup
  const element = document.querySelector("m-card")
  if (!(element instanceof MCard)) throw new Error("Card was not upgraded")
  return element
}

function closeButton(element: MCard): HTMLButtonElement {
  return element.querySelector<HTMLButtonElement>("[data-m-card-close]")!
}

describe("standalone Card", () => {
  it("wraps free-form content without cloning nodes or discarding listeners", () => {
    const element = document.createElement("m-card") as MCard
    ...
    const content = element.querySelector("[data-m-card-content]")!
  ...
  it("supports all six native anatomy regions and moves a direct extra into the header", () => {
    const element = card(`<m-card closable role="region" aria-labelledby="heading">
      <div data-m-card-cover><img alt="Cover"></div>
      <header data-m-card-header><h2 id="heading">Heading</h2></header>
      <div data-m-card-header-extra><button type="button">Extra</button></div>
      <section data-m-card-content>Content</section>
      <footer data-m-card-footer>Footer</footer>
      <div data-m-card-action>Action</div>
    </m-card>`)
    const header = element.querySelector("header")!
    expect(header.querySelector("[data-m-card-header-extra]")).not.toBeNull()
  ...
  it("generates a safe text title without inferring heading level or replacing body content", () => {
    ...
    const title = element.querySelector("[data-m-card-title]")!
    expect(title.textContent).toBe("<img src=x onerror=alert(1)>")
    expect(title.querySelector("img")).toBeNull()
    expect(title.hasAttribute("role")).toBe(false)
    element.title = "Changed"
    expect(element.querySelector("[data-m-card-title]")).toBe(title)
    expect(title.textContent).toBe("Changed")
    expect(element.querySelector("[data-m-card-content]")).toBe(content)
    element.removeAttribute("title")
    expect(element.querySelector("[data-m-card-header]")).toBeNull()
    expect(element.querySelector("[data-m-card-content]")).toBe(content)
  })

  it("gives authored headers precedence over the title convenience", () => {
    const element = card('<m-card title="Fallback"><header data-m-card-header><h2>Authored</h2></header><p>Body</p></m-card>')
    const heading = element.querySelector("h2")!
    element.title = "Changed fallback"
    expect(element.querySelector("[data-m-card-title]")).toBeNull()
  ...
  it("adopts late authored headers and keeps close last while preserving extra nodes", async () => {
    const element = card('<m-card title="Fallback" closable><div data-m-card-header-extra><button type="button">Extra</button></div>Body</m-card>')
    const extra = element.querySelector("[data-m-card-header-extra]")!
    const close = closeButton(element)
    const header = document.createElement("header")
    header.setAttribute("data-m-card-header", "")
    const heading = document.createElement("h2")
    heading.textContent = "Authored"
    header.append(heading)
    element.prepend(header)
    await Promise.resolve()
    expect(element.querySelectorAll("[data-m-card-header]")).toHaveLength(1)
  ...
  it("adopts late explicit content and preserves all previous free-form nodes", async () => {
    ...
    content.setAttribute("data-m-card-content", "")
    content.textContent = "New"
    element.append(content)
    await Promise.resolve()
    expect(element.querySelectorAll("[data-m-card-content]")).toHaveLength(1)
  ...
  it("places a generated header after cover and default content before footer/action", () => {
    const element = card('<m-card title="Title"><div data-m-card-cover>Cover</div><footer data-m-card-footer>Footer</footer><div data-m-card-action>Action</div>Body</m-card>')
  ...
  it("preserves inert templates without cloning or activating them", () => {
    const element = card('<m-card><template id="content-template"><button type="button">Inert</button></template>Body</m-card>')
  ...
  it("emits one bubbling cancellable close intent without hiding or removing the card", () => {
    const element = card('<m-card closable title="Dismissible">Body</m-card>')
    const close = closeButton(element)
  ...
  it("uses a type=button close control that never submits an enclosing form", () => {
    const element = card('<form><m-card closable>Body</m-card></form>')
  ...
  describe("audited Card styles", () => {
    ...
    it("keeps shared typography and focus overrides ahead of standalone fallbacks", () => {
      expect(css).toContain("var(--m-card-font-size, var(--m-font-size, 14px))")
      expect(css).toContain("var(--m-card-line-height, var(--m-line-height, 1.6))")
      expect(css).toContain("var(--m-card-focus-color, var(--m-focus-ring, #2080f080))")
      expect(css).toContain("var(--m-card-target-color, var(--m-color-primary, var(--_m-card-primary, #18a058)))")
    })
    ...
    it("does not map Card colors to unequal legacy palette roles or overwrite public tokens", () => {
      ...
      expect(css).toContain("var(--m-card-background, var(--_m-card-surface, #fff))")
      expect(css).toContain("var(--m-card-border-color, var(--_m-card-border, #efeff5))")
      expect(css).toContain("var(--m-card-title-weight, 500)")
      expect(css).toContain("var(--m-card-radius, 3px)")
    })
    ...
    it("preserves measured asymmetric size geometry and unsegmented region spacing", () => {
      expect(css).toContain("--_m-card-top: 19px")
      expect(css).toContain("--_m-card-bottom: 20px")
      ...
      expect(css).toContain("padding: 0 var(--m-card-padding, var(--_m-card-padding)) var(--m-card-padding, var(--_m-card-bottom))")
    ...
    it("keeps a font-independent close mark, expanded state paint and reduced-motion opt-out", () => {
      expect(css).toContain("width: var(--m-card-close-size, 18px)")
      expect(css).toContain("inset: -2px")
      expect(css).toContain("transform: rotate(-45deg)")
      expect(css).toContain("@media (prefers-reduced-motion: reduce)")
      expect(css).toContain('[data-m-card-close]::before { transition: none; }')
    })
  })
  ...
  it("removes generated-only headers and close listeners when closable is unset", () => {
    ...
    expect(element.querySelectorAll("[data-m-card-close]")).toHaveLength(1)
    closeButton(element).click()
    expect(intent).toHaveBeenCalledOnce()
  })

  it("retains authored header content when removing the generated close button", () => {
    const element = card('<m-card closable><header data-m-card-header><h2>Heading</h2><button type="button">Action</button></header>Body</m-card>')
    const header = element.querySelector("header")!
  ...
  it("disconnects observers/listeners and reconnects without duplication or lost identity", async () => {
    ...
    await Promise.resolve()
    expect(intent).not.toHaveBeenCalled()
    expect(element.querySelector("[data-m-card-title]")?.textContent).toBe("Before")
    document.body.append(element)
    expect(closeButton(element)).toBe(close)
    expect(element.querySelector("[data-m-card-title]")?.textContent).toBe("After")
  ...
  it("handles replaced children without resurrecting removed authored content", async () => {
    const element = card('<m-card closable title="Fallback"><header data-m-card-header>Old header</header><div data-m-card-content><input value="Old"></div></m-card>')
    const close = closeButton(element)
    const oldInput = element.querySelector("input")!
    element.innerHTML = '<header data-m-card-header>New header</header><section data-m-card-content>New content</section>'
    await Promise.resolve()
    expect(closeButton(element)).toBe(close)
  ...
  it("regenerates removed close placement and detects emptied default content", async () => {
    ...
    expect(closeButton(element)).toBe(close)
    element.closable = false
    element.querySelector("[data-m-card-content]")!.replaceChildren()
    await Promise.resolve()
  ...
  it("recognizes late region attributes and direct text character changes", async () => {
    ...
    await Promise.resolve()
    expect(text.parentElement).toBe(element.querySelector("[data-m-card-content]"))
  })

  it("does not turn a nested card close into a second parent close intent", () => {
    const outer = card('<m-card closable title="Outer"><m-card closable title="Inner">Body</m-card></m-card>')
    const inner = outer.querySelector("m-card") as MCard
    const intent = vi.fn()
  ...
  it("upgrades pre-definition properties and keeps templates/native regions intact", () => {
    ...
    const element = document.querySelector("test-late-card") as MCard
    ...
    customElements.define("test-late-card", class extends MCard {})
    expect(element.querySelector("[data-m-card-title]")?.textContent).toBe("Late")
  ...
  it("preserves rich Card registration when loading the legacy compound elements", () => {
    ...
    expect(customElements.get("m-card")).toBe(MCard)
    const element = card("<m-card closable><m-card-header>Heading</m-card-header><m-card-content>Content</m-card-content><m-card-footer>Footer</m-card-footer></m-card>")
    expect(customElements.get("m-card-header")).toBeDefined()
    expect(element.hasAttribute("structured")).toBe(true)
    expect(element.querySelector("m-card-header > [data-m-card-close]")).not.toBeNull()
  })
})
