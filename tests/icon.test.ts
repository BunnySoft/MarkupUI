...
describe("CSS-only Icon and IconWrapper", () => {
  it("exports only CSS with no fake runtime or icon library", () => {
    ...
    expect(customElements.get("m-icon")).toBeUndefined()
    expect(customElements.get("m-icon-wrapper")).toBeUndefined()
  })

  it("preserves SVG namespaces, viewports, titles, descriptions and paint attributes", () => {
    document.body.innerHTML = '<svg class="m-icon" role="img" aria-labelledby="title desc" viewBox="0 0 40 20" preserveAspectRatio="xMidYMid meet"><title id="title">Native artwork</title><desc id="desc">Description</desc><path d="M0 0h20" fill="none" stroke="#123456" stroke-width="2"></path><circle cx="20" cy="10" r="5" fill="red"></circle></svg>'
  ...
  it("does not contain blanket fill/stroke or nested-SVG resets", () => {
    ...
    document.body.innerHTML = '<svg class="m-icon" viewBox="0 0 40 40"><svg width="12" height="6" viewBox="0 0 12 6"><rect width="12" height="6" fill="blue"></rect></svg></svg>'
  ...
  it("preserves standalone and decorative naming without creating duplicate roles", () => {
    document.body.innerHTML = '<span class="m-icon" aria-hidden="true">+</span><svg class="m-icon" role="img" aria-label="Meaningful" viewBox="0 0 10 10"></svg>'
    install()
  ...
  it("keeps native button/link attributes, event listeners and keyboard defaults", () => {
    document.body.innerHTML = '<button type="button" aria-label="Save"><span class="m-icon" aria-hidden="true">✓</span></button><a class="m-icon-wrapper" href="#next" target="_blank" rel="noopener" aria-label="Next"><span class="m-icon" aria-hidden="true">→</span></a>'
  ...
  it("does not change unclassified SVG or document direction/language", () => {
    document.body.innerHTML = '<div dir="rtl" lang="ar"><svg id="outside" width="70" height="30" viewBox="0 0 70 30"><rect fill="#556677"></rect></svg><span class="m-icon">◇</span></div>'
  ...
  it("keeps native image source/alt/intrinsic attributes and hidden states", () => {
    document.body.innerHTML = '<span class="m-icon" hidden><img src="original.svg" alt="" width="40" height="20"></span><span class="m-icon-wrapper" hidden>Hidden</span>'
    const image = document.querySelector("img")!
    const before = image.outerHTML
    install()
    expect(image.outerHTML).toBe(before)
    expect(getComputedStyle(document.querySelector(".m-icon")!).display).toBe("none")
    expect(getComputedStyle(document.querySelector(".m-icon-wrapper")!).display).toBe("none")
  })

  it("defines depth and forced-color fallback without keyframes or disabled-control emulation", () => {
    document.body.innerHTML = '<span class="m-icon" data-depth="5">◇</span>'
    install()
    expect(css).toContain('@media (forced-colors: active)')
    expect(css).not.toContain("forced-color-adjust")
    expect(css).not.toContain("@keyframes")
    expect(css).not.toContain("pointer-events: none")
    expect(document.querySelector(".m-icon")?.hasAttribute("role")).toBe(false)
  })
  ...
  it("uses the verified inline-box and baseline defaults without forcing native font style", () => {
    ...
    document.body.innerHTML = '<i class="m-icon">A</i><span class="m-icon">A</span>'
    install()
  ...
  it("encodes the pinned light and dark depth values with retained author overrides", () => {
    for (const [depth, light, dark] of [[1, ".82", ".9"], [2, ".72", ".82"], [3, ".38", ".52"], [4, ".24", ".38"], [5, ".18", ".28"]]) {
      expect(css).toContain(`opacity: var(--m-icon-depth-${depth}, var(--_m-icon-depth-${depth}, ${light}))`)
      expect(css).toContain(`--_m-icon-depth-${depth}: ${dark}`)
    }
    expect(css).toContain("--_m-icon-color: light-dark(#000, #fff)")
    expect(css).toContain("var(--m-icon-color, var(--_m-icon-color, inherit))")
    expect(css).toContain('.m-icon:not(svg[color])')
  })
  ...
  it("uses theme-correct wrapper contrast and transitions without adding default border geometry", () => {
    ...
    expect(css).toContain(".m-icon[data-depth], .m-icon-wrapper { transition: none; }")
  ...
  it("preserves authored rotation, SVG color/opacity and explicit sizes rather than inventing type/rotation props", () => {
    document.body.innerHTML = '<svg class="m-icon" color="#a04080" opacity=".45" style="transform:rotate(30deg);font-size:28.5px" viewBox="0 0 20 20" preserveAspectRatio="none"><rect fill="currentColor" width="20" height="20"></rect></svg>'
    const svg = document.querySelector("svg")!
    const before = svg.outerHTML
    install()
    expect(svg.outerHTML).toBe(before)
  ...
  it("keeps all native wrapper action types and successful form controls unchanged", () => {
    document.body.innerHTML = '<form><input name="value" value="before"><button class="m-icon-wrapper" type="reset" aria-label="Reset">R</button><button class="m-icon-wrapper" type="submit" name="action" value="save">S</button></form>'
    install()
