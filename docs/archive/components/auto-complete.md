...
HTTP client or runtime dependency. Legacy `m-autocomplete`/`forms.ts` remain unchanged.

## Default-style audit — 2026-09-11

The [isolated rendered audit](../styling/components/auto-complete.md) corrects
...
<p class="m-auto-complete__status" id="suggestion-status">Authored suggestions.</p>
```
...
| [Reference dispositions](../naive/components/auto-complete.md) | All 46 original identities and explicit source supplements |
...
For `.m-auto-complete__field` without `.m-input`, the authored field wrapper
...
| `data-size` on `.m-auto-complete` | Height | Font | Inline padding |
| --- | ---: | ---: | ---: |
...
Select `data-m-theme="light|dark"` on an ancestor. The field uses a matching native
...
Public standalone tokens use the `--m-auto-complete-` prefix: `height`, `font`,
`pad`, `color`, `background`, `border`, `focus`, `disabled` and `placeholder`.
Family may use shared `--m-font-family`. Data-size rules set private defaults,
...
When the field also has `.m-input`, Auto Complete deliberately excludes it from
...
normal screen focus and `.m-input` ownership are unchanged.
...
[Input print follow-up](../styling/components/input.md);
...
- `m:auto-complete-results` is a nonbubbling **query-result** event on the input:
...
the public query and dispatch `m:auto-complete-error` on the input with `{ error, query }`.
...
- ESM/classic/Input/Form and legacy `m-autocomplete` registration coexisted. No Safari/
