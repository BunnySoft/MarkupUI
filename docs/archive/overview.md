...
[platform architecture](../architecture/README.md) introduces `m-*` as the
public Web prefix. The ordered
[Element Contracts](../elements/README.md) define the new unprefixed logical
element classes, `ViewElement` Web adapter, properties, content, actions, events,
states and behavior. The canonical model contains elements, regions, bindings, templates,
actions, capabilities and fallbacks rather than raw HTML.

The agreed [full component rewrite](../architecture/04-rewrite.md)
moves every retained Web UI component to `ViewElement`, including its behavior, content,
registration, styling and lifecycle. The first seven-component batch is the first milestone.
[Class-owned metadata](../architecture/05-meta.md) is one part:
`Avatar.meta` will expose reflected `ElementMeta`. This metadata API is planned; current
code still exposes handwritten definitions and `.definition`.
...
Avatar is a standalone component: import `@dataengine/markup-ui/avatar` and link its
`@dataengine/markup-ui/avatar/style.css` stylesheet. The aggregate no longer registers a
second basic Avatar; the two entries can be loaded in either order.
...
The component list below describes the current `m-*` Web surface, including standalone
entries. The Element Contract rewrite starts with Avatar, then Button through Dropdown.
...
[ordered data binding and templates guide](binding/README.md). It covers the
...
See the [foundation and loading guide](naive/foundations.md) for source locations,
...
The [migration plan](naive/migration-plan.md) organizes phases, tasks and status.
The [Naive UI component index](naive/index.md) tracks component and property-level
planning. Its [architecture](naive/architecture.md) keeps
...
resolution; this is not full Naive UI API parity. The [component index](naive/index.md)
links every property-level record. Example standalone usage records:
[Avatar and Avatar Group](components/avatar.md),
[Button and Button Group](components/button.md), and [Card](components/card.md).
...
[Read the current design](../README.md).
