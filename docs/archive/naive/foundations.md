...
## Authoring sources and generated modules

| Maintained source | Generated module | External distribution |
| --- | --- | --- |
...
Edit the CSS/JSON sources, **not the generated TypeScript modules**. The existing
`pnpm build` regenerates modules through
[`scripts/styles.mjs`](../../../scripts/styles.mjs) before TypeScript compilation.
Modules are checked in so ordinary source imports/tests remain available after checkout;
they are generated runtime data, not a second maintained stylesheet or palette.
Generation normalizes checkout line endings and does not rewrite unchanged modules.

The three runtime CSS strings are checked against reviewed SHA-256 baselines.
Both built-in theme maps preserve all 31 original string tokens and their values.
The modules support native JavaScript imports and style installation without a
